<?php
    ...
    $pdf->MultiCell(0,5,mb_convert_encoding(strtoupper("Lavandería Sapito"),'ISO-8859-1', 'UTF-8'),0,'C',false);
    $pdf->SetFont('Arial','',9);
    ...
    $pdf->MultiCell(0,5,mb_convert_encoding("Fecha: ".date("d/m/Y", strtotime(date('Y-m-d')) )." ".date('H:i:s'),'ISO-8859-1', 'UTF-8'),0,'C',false);
    //$pdf->MultiCell(0,5,utf8_decode("Caja Nro: 1"),0,'C',false);
    $pdf->MultiCell(0,5,mb_convert_encoding($empleado ,'ISO-8859-1', 'UTF-8'),0,'C',false);
    $pdf->SetFont('Arial','B',10);

    //$numero = random_int(1, 99);
    //$letra_aleatoria = chr(rand(65, 90));
    $ultimoId = mysqli_insert_id($conexion);
    $consultaFolioNota = "SELECT * FROM ctl_ventapedidos WHERE id_ctl_ventapedidos = $idNota";
    ...
    if (mysqli_num_rows($stmtFolioNota) > 0) {
      while ($fila = mysqli_fetch_array($stmtFolioNota)) {  
        ...
        $pdf->MultiCell(0,5,mb_convert_encoding($numeroTicket,'ISO-8859-1', 'UTF-8'),0,'C',false);
        }
    } else {
